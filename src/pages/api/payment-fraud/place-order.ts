...
import {MIN_CONFIDENCE_SCORE} from "../../../server/const";
...
export const OrdersDbModel = sequelize.define<OrderAttributes>('orders', {
  visitorId: {
    type: DataTypes.STRING,
  },
  product: {
...
OrdersDbModel.sync({ force: false });

export type Orders = Attributes<OrderAttributes>;
  ...
  for (const ruleCheck of ruleChecks) {
    ...
    if (result) {
      if (result.severity == 'success'){
        await logOrder(visitorId, 'iPhone 13');
      }
...
export const checkForBot: RuleCheck = (eventResponse) => {
...
const checkOneProductPerVisitor: RuleCheck = async (eventResponse) => {
  const orderCountQueryResult = await OrdersDbModel.findAndCountAll({
    where: {
      visitorId: eventResponse.products?.identification?.data?.visitorId,
      timestamp: {
  ...
  if (orderCountQueryResult.count > 0) {
    return new CheckResult(
      'You have already ordered a phone before...',
      messageSeverity.Error,
