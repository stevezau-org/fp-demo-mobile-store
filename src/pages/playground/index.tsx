...
function Playground() {
  ...
  const locationName = useMemo<string>(() => {
    return getLocationName(identificationEvent?.products?.ipInfo?.data?.v4);
  }, [identificationEvent]);
  ...
  const { latitude, longitude } = identificationEvent?.products?.ipInfo?.data?.v4?.geolocation ?? {};
  ...
  const identificationSignals: TableCellData[][] = [
    ...
    [
      {
        content: <DocsLink href='https://dev.fingerprint.com/docs/useful-timestamps#definitions'>First seen</DocsLink>,
      },
      {
        content: agentResponse?.firstSeenAt.global ? timeAgoLabel(agentResponse?.firstSeenAt.global) : 'Unknown',
      },
    ],
    [
      {
